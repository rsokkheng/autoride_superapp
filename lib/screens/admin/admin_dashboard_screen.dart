...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('$total ${AppLocalizations.of(context).pendingActionsSuffix}', style: const TextStyle(color: AppTheme.warning, fontWeight: FontWeight.w700, fontSize: 13)),
        ]),
        const SizedBox(height: 10),
        Wrap(spacing: 8, runSpacing: 6, children: [
          if (approvals   > 0) _AlertChip(label: '$approvals ${AppLocalizations.of(context).driverApprovalsSuffix}', onTap: () => _push(const AdminDriverApprovalScreen())),
          if (withdrawals > 0) _AlertChip(label: '$withdrawals ${AppLocalizations.of(context).withdrawalsAdmin}',    onTap: () => _push(const AdminWithdrawalsScreen())),
          if (tickets     > 0) _AlertChip(label: '$tickets ${AppLocalizations.of(context).ticketsSuffix}',            onTap: () => _push(const AdminSupportScreen())),
          if (topups      > 0) _AlertChip(label: '$topups ${AppLocalizations.of(context).topUps}',             onTap: () => _push(const AdminTopupsScreen())),
        ]),
      ]),
    );
  }
  ...
  Widget _buildOverviewGrid() {
    ...
    final cards = [
      _StatCard(icon: Icons.person_outline,         label: l.passengersStat,    value: _str('passengers'),       color: const Color(0xFF2196F3)),
      _StatCard(icon: Icons.drive_eta_outlined,     label: l.driversTotal,      value: _str('drivers_total'),    color: AppTheme.accent),
      _StatCard(icon: Icons.wifi_tethering_rounded, label: l.onlineDrivers,     value: _str('drivers_online'),   color: AppTheme.success),
      _StatCard(icon: Icons.pending_outlined,       label: l.pendingDrivers,    value: _str('drivers_pending'),  color: AppTheme.warning),
      _StatCard(icon: Icons.directions_car_rounded, label: l.ridesToday,        value: _str('rides_today'),      color: AppTheme.accent),
      _StatCard(icon: Icons.directions_car_filled,  label: l.activeRides,       value: _str('rides_active'),     color: AppTheme.success),
      _StatCard(icon: Icons.local_shipping_rounded, label: l.deliveriesToday,   value: _str('deliveries_today'), color: AppTheme.accentOrange),
      _StatCard(icon: Icons.attach_money_rounded,   label: l.revenueToday,      value: _khr('revenue_today'),    color: AppTheme.gold),
      _StatCard(icon: Icons.show_chart_rounded,     label: l.revenueWeek,       value: _khr('revenue_week'),     color: AppTheme.gold),
      _StatCard(icon: Icons.trending_up_rounded,    label: l.growth,            value: '${_str('revenue_growth_pct')}%', color: AppTheme.success),
      _StatCard(icon: Icons.support_agent_rounded,  label: l.openTickets,       value: _str('support_open'),     color: AppTheme.danger),
      _StatCard(icon: Icons.security_rounded,       label: l.safetyIncidents,   value: _str('safety_incidents'), color: AppTheme.danger),
    ];

    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(l.overview, style: TextStyle(color: context.appTextPrimary, fontSize: 17, fontWeight: FontWeight.w700)),
  ...
  Widget _buildManagementGrid() {
    final l = AppLocalizations.of(context);
    final sections = [
      (l.users, Icons.people_outline, const Color(0xFF2196F3), false),
      (l.drivers, Icons.drive_eta_outlined, const Color(0xFF00B14F), false),
      (l.ridesAdmin, Icons.directions_car_outlined, const Color(0xFF00B14F), false),
      (l.deliveriesAdmin, Icons.local_shipping_outlined, const Color(0xFFFF6B2B), false),
      (l.withdrawalsAdmin, Icons.account_balance_outlined, const Color(0xFFFFD700), false),
      (l.topUps, Icons.add_card_rounded, const Color(0xFF4CAF50), false),
      (l.supportAdmin, Icons.support_agent_outlined, const Color(0xFF9C27B0), false),
      (l.transactionsAdmin, Icons.receipt_long_outlined, const Color(0xFF607D8B), false),
      (l.fareRules, Icons.price_change_outlined, const Color(0xFF00B14F), false),
      (l.surgeZones, Icons.bolt_outlined, const Color(0xFFFF6B2B), false),
      (l.pricingAdmin, Icons.tune_rounded, const Color(0xFF00BCD4), false),
      (l.bannersAdmin, Icons.image_outlined, const Color(0xFF9C27B0), false),
      (l.safetyAdmin, Icons.security_rounded, const Color(0xFFF44336), false),
    ];
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(l.management, style: TextStyle(color: context.appTextPrimary, fontSize: 17, fontWeight: FontWeight.w700)),
      const SizedBox(height: 12),
      GridView.builder(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
